...
class misc {
    ...
    */
     ...
     * @param string $bd_en_pw 加密过的百度密码
     * @param string $verifycode 验证码
     * @param string $vcodestr 验证字符
     * @return array [0成功|-1网络请求失败|-2json解析失败|-3表示需要验证码或验证码错误|其他为百度提供的错误代码, 成功为BDUSS|需要验证码则返回vcodestr|其他错误返回百度提供的错误信息, 如果登陆成功，返回百度用户名|如果需要验证码，则此处返回验证图片地址 ]
     */
    public static function loginBaidu( $bd_name , $bd_en_pw , $verifycode = '', $vcodestr = '') {
        $x = new wcurl('https://wappass.baidu.com/wp/api/login');
        $x->addCookie("BAIDU_WISE_UID=wpass_1501994223170_178");
        $p = array(
            'username'   => $bd_name,
            'password'   => $bd_en_pw,
        );
        ...
        switch($v['errInfo']['no']) {
            case '0': //成功
                return array(0, $bduss, $bd_name);
                break;
            case "400101": //开启登录保护
                return array((int)$v['errInfo']['no'], "您的帐号已开启登录保护服务, 登录前请先进行安全验证。");
                break;
            case "400023": //需要验证手机或邮箱
                return array((int)$v['errInfo']['no'], "您的帐号登录异常, 验证手机或邮箱之后才能登录, 网址: ").$v['data']['gotoUrl'];
                break;
            case '500001': //需要验证码或验证码输入错误
            case '500002':
                return array(-3, $v['data']['codeString']);
                break;

            default: //其他错误
                return array((int)$v['errInfo']['no'], $v['errInfo']['msg']);
                break;
        }
    }
